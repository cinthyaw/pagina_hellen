...
if (isset($_POST) && isset($_POST['productos'])) {
	$db->guardarDestacados($_POST['productos']);
}
					...
					<form method="post">
						...
						<table class="table table-bordered">
							...
							foreach ($productos as $p) {
								?>
								<tr>
									<td class="center-check"><input type="checkbox" value="<?=$p['id']?>" name="productos[]" <?=$p['checked'] ? 'checked' : ''?>></td>
									<td><?=$p['nombre']?></td>
									<td><?=$p['descripcion']?></td>
									<td><?=$p['categoria']?></td>
